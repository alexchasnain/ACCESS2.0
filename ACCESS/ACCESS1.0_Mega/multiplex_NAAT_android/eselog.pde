boolean eselogConnected = false;
boolean eselogInit = false;
...
int addr;                // holds address of register for reading/writing on eselog
int data;                // holds data for writing to eselog
String args[];
String strbuff;          // temporary buffer for building modbus string

void setupESELog(){
    println("Initializing ESELog.");
    
    modbus("write,cycles,1");     // initialize to one cycle per measurement
    modbus("write,led1_current,"+ led1_current);
    modbus("write,led2_current,"+ led2_current);
    modbus("write,method_type," + method_type);
    modbus("write,led1_on_delay," + led1_on_delay);
    modbus("write,led2_on_delay," + led2_on_delay);
    modbus("write,led1_off_delay," + led1_off_delay);
    modbus("write,led2_off_delay," + led2_off_delay);
...
void setMethodType() {
    if (LED1.state) {
        ...
        } else if (Det1.state) { // only LED 1 on
            if (!Det2.state) method_type = 8; // S_E1D1
            else method_type = 4;    // E1D1 + E1D2
        } else if (Det2.state) { 
            method_type = 9;        // S_E1D2
        }
    } else if (LED2.state) { // only LED 2 on
        method_type = 10; // S_E2D2
    }
    modbus_str = modbus("write,method_type,"+method_type);
}

//called in modbus(String msg) to send modbus code to arduino
void modbusSend(String modbus_str){
    if(eselogConnected){
        ardSend(modbus_str,0);
    }
    else println("ESELog not connected - didn't send: " + modbus_str);
}
