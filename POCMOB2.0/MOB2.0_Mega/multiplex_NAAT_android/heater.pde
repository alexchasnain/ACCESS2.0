void test_heater(float temp, float time, int heater) {
    ...
    n_plots = 6;  // 6 fluo channels 
    fluo_plotter = new plot2D(plotx, ploty-ploth/2, plotw, ploth, n_plots);
...
void cycle() {
    ...
    ardSend("cycle("+pcrAnnTemp_cal+","+pcrAnnTime+","+pcrDenTemp_cal+","+pcrDenTime+","+cycleN+","+hotsTime+","+detect+")",0);
    serialMode = 1; // data logging initiated with asterisk
    ...
    n_plots = 6;  // just temperature vs. time    
    fluo_plotter = new plot2D(plotx, ploty-ploth/2, plotw, ploth, n_plots);
    fluo_plotter.pt_size = 4;
    fluo_plotter.labels = true;
    fluo_plotter.xlabel = "Cycle";
...
void bisCon() {
    //bisCon_flag = true;
    ardSend("bisCon("+bisDigTemp+","+bisDigTime+","+bisDenTemp+","+bisDenTime+","+bisConTemp+","+bisConTime+","+bisBindTemp+","+bisBindTime+")",0);
